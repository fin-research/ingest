...
- `omo` Workflow 的业务步骤、重试与通知边界见[公开市场播报](modules/open-market.md)。
...
2026-09-21 创建原生 Workflow schedule 的实际 API 返回 403、code `10208`（账户未启用所需计划）；生产继续使用 Worker Cron。再次启用前须重查当前账户资格和 API 响应，本地 dry-run 不代表能创建。
...
- 新命令或交付流程更新本文件。
- 调度依赖更新 `ARCHITECTURE.md`，Workflow 步骤更新目标模块；新增模块在 [AGENTS](../AGENTS.md#context-routing) 登记文档入口。
- D1 幂等与 migration 更新 `DATABASE.md`；跨仓库表、归档协议与 AI 变更更新[项目组对应共享专题](../../eastmoney/AGENTS.md#context-routing)。
