...
- Wrangler 必须为 v4；配置使用 `wrangler.jsonc`，部署前先跑 `pnpm deploy:dry`，正式部署用 `pnpm deploy:worker`。
- Worker 与 D1 数据库固定名为 `ingest`；R2 存储桶固定为 `article`，Workflow 名固定为 `article`，AI Search 固定为 `default/finance`。
...
- Workflow 固定为三个基础步骤：获取 DM 文章详情、写 R2、写 AI Search；若原文是 `mp.weixin.qq.com`，在获取详情后增加独立的公众号下载步骤。获取 DM 详情时把原文 `link` 幂等写入 D1；公众号步骤优先直连下载并按 kb 口径转换 Markdown，再删除图片与链接 URL（链接锚文本保留），并清除风险披露及后文；下载或解析失败则使用 DM 正文。步骤必须幂等，所有 Promise 必须 await。
...
5. `pnpm test`
6. `pnpm deploy:dry`
7. 部署前先对远端 `ingest` D1 执行 `pnpm db:migrate:remote`；部署后检查 `/health` 和 Cron 日志。
