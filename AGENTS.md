...
- Workflow 在下载和归档之间固定增加 Gemma 4 特征抽取与 D1 存储步骤，随后写 R2、写 AI Search；若原文是 `mp.weixin.qq.com`，在获取详情后增加独立的公众号下载步骤。获取 DM 详情时把原文 `link` 幂等写入 D1；公众号步骤优先直连下载并按 kb 口径转换 Markdown，再删除图片与链接 URL（链接锚文本保留），并清除风险披露及后文；下载或解析失败则使用 DM 正文。Gemma 4 使用 Workers AI binding 并经 AI Gateway 记录调用，缓存关闭；输入和有效输出不得截断。步骤必须幂等，所有 Promise 必须 await。
