...
- `src/open-market.ts`：工作日 09:20–09:25 的公开市场操作播报，两步分别获取投放公告、查询到期并计算净额，使用 Workflow 原生重试。
...
- Cron 固定为 `*/5 0-9 * * MON-FRI`（UTC），即北京时间工作日 `[08:00, 18:00)` 每 5 分钟；09:20 同时启动当日 `OmoWorkflow`（omo），两个 step 使用原生恒定 15 秒失败重试、最多重试 20 次，09:25 截止未完成则明确失败。
