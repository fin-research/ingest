-- article.id is the DM sentimentId and is the canonical article identity.
...
SELECT
  ...
  published_at,
  discovered_at,
  COALESCE(feature_extracted_at, discovered_at),
  link,
  author,
  summary,
  importance,
  CASE
    WHEN feature_prompt_version IS NULL THEN NULL
    WHEN feature_prompt_version IN ('article-features-v3', 'v1') THEN 'v1'
    ELSE 'v2'
  END
FROM article;
