...
7. interpretation 说明事实背后的定价或传导含义。impact 必须是一至两句可直接用于投资判断的明确观点，写清主要受影响的资产或行业、方向以及关键机制或成立条件。每个分句都必须至少包含明确方向、仓位建议或可验证的条件结果，不能先写空泛判断再补一句方向。优先使用“利多/利空、推高/压低、提振/压制、扩大/收窄、增加/降低”等有方向的词。任何包含“影响/引导/改变定价逻辑”“引导走势”“影响整体走势”“影响风险偏好”“值得关注”“带来影响”“存在不确定性”等空泛分句的 impact 都不合格，即使同一句后半段有方向也必须删除空泛分句并整体重写。例如：“影响债市利率定价逻辑，引导长端利率走势”应改为“隔夜逆回购缓解税期资金压力，短期利多长端利率债并压低收益率”；“引导债市利率定价逻辑，限制利率过度下行空间”应改为“央行容忍约1.30%的DR001，资金利率进一步下行空间受限，短端利率债的宽松交易将降温”；“影响科技板块整体走势及市场风险偏好”应改为“光通信权重企稳可提振科技板块风险偏好，若放量转跌则将加大成长股抛压”。若原文证据不足以形成明确观点，就不要输出该 keyword，而不是给出模糊 impact。相较其他市场优先分析权益和固收，但不强行覆盖两个市场。输出 JSON 前逐条复查 impact，发现任一空泛分句必须重写。
8. summary 仅用一句、目标 45-55 个汉字概括核心结论、最关键依据和首要市场影响，硬性不得超过 60 个汉字；删除背景铺陈、标题复述和次要细节。输出 JSON 前计算字数，超过 60 字必须压缩重写。
